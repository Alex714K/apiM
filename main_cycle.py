...
if __name__ == "__main__":
    ...
    schedule.every().day.at("13:45").do(API.start, name_of_sheet='stocks', dateFrom='2019-06-20')
    schedule.every().day.at("13:50").do(API.start, name_of_sheet='orders_1mnth', dateFrom='1mnth')
    schedule.every().day.at("13:55").do(API.start, name_of_sheet='orders_1week', dateFrom='1week')
    schedule.every().day.at("14:00").do(API.start, name_of_sheet='orders_2days', dateFrom='2days')
    schedule.every().day.at("14:05").do(API.start, name_of_sheet='orders_today', dateFrom='today', flag='1')
    schedule.every().day.at("14:10").do(API.start, name_of_sheet='tariffs_boxes', date='tariffs')
    schedule.every().day.at("14:15").do(API.start, name_of_sheet='tariffs_pallet', date='tariffs')
