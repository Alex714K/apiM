...
def Ozon_cycle():
    ...
    # Вечер гранд
    schedule.every().day.at("10:20").do(API.start, 'analytics', 'grand', 'Ozon')
    schedule.every().day.at("10:25").do(API.start, 'stock_on_warehouses', 'grand', 'Ozon')
    schedule.every().day.at("10:30").do(API.start, 'products', 'grand', 'Ozon')
    schedule.every().day.at("10:35").do(API.start, 'orders_1mnth', 'grand', 'Ozon')
    schedule.every().day.at("10:40").do(API.start, 'orders_1week', 'grand', 'Ozon')
    schedule.every().day.at("10:45").do(API.start, 'orders_2days', 'grand', 'Ozon')
